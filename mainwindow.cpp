...
void MainWindow::Update_CPU_Line()
{
    ...
    while(count < ylist.size()-1)
    {
         ...
         painter.drawLine(18*count, 390-3.9*(ylist.value(count)),18*(count+1),390-3.9*(ylist.value(count+1)));
...
void MainWindow::Update_Mem_Line()
{
    ...
    while(count<(ylistM.size()-1)){
        painter.drawLine(18*count, 390-3.9*(ylistM.value(count)), 18*(count+1), 390-3.9*(ylistM.value(count+1)));
        count++;
    }
